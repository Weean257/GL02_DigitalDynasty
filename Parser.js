...
function parseDataLine(line) {
    ...
    const participants = components[2].includes('=') ? components[2].split('=')[1].trim() : null;
    const timeInfo = components[3].split(' ');
    const day = timeInfo[0].includes('=') ? timeInfo[0].split('=')[1].trim() : null;
    const hours = timeInfo[1].trim();
    const frequency = components[4].includes('=') ? components[4].split('=')[1].trim() : null;
    const room = components[5].includes('=') ? components[5].split('=')[1].split('//')[0].trim() : null;
